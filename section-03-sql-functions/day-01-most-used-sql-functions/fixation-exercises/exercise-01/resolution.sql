...
SELECT 
    ROUND(MAX(salary), 2) AS max_salary, ROUND(MIN(salary), 2) AS min_salary, ROUND(SUM(salary), 2) AS sum_salary, ROUND(AVG(salary), 2) AS average_salary
FROM
    hr.employees;
...
SELECT 
    job_id, SUM(salary) AS sum_salary
FROM
    hr.employees
GROUP BY job_id;
...
SELECT 
    employee_id, first_name, RIGHT(hire_date, 2) AS hire_day
FROM
    hr.employees;
